...
import { isFunctionExtension } from '../models/app/extensions.js';
...
import { convertSpecificationsToTemplate, getTypesExternalIdentitifier, getTypesExternalName, } from '../models/app/template.js';
...
async function generate(options) {
    ...
    const app = await loadApp({ directory: options.directory, specifications: specificationTypes });
...
async function getTemplateSpecifications(options) {
...
async function buildPromptOptions(templateSpecifications, app, options) {
...
function checkLimits(templateSpecifications, app) {
    const iterateeFunction = (spec) => {
...
function limitReached(app, type) {
...
function buildGenerateOptions(promptAnswers, app, options) {
    return promptAnswers.extensionContent.map((extensionContent) => {
        return {
            ...
            extensionFlavor: extensionContent.extensionFlavor,
...
function renderSuccessMessages(generatedExtensions, packageManager) {
    generatedExtensions.forEach((extension) => {
        const formattedSuccessfulMessage = formatSuccessfulRunMessage(extension.specification, extension.directory, packageManager);
        renderSuccess(formattedSuccessfulMessage);
    });
}
function findTemplateSpecification(type, specifications) {
    // To support legacy extensions specs, we need to check both the identifier and the external identifier
    return specifications.find((spec) => spec.types.some((extension) => extension.identifier === type || extension.externalIdentifier === type));
}
function validateExtensionFlavor(templateSpecification, flavor) {
    if (!flavor || !templateSpecification)
        return;
    const possibleFlavors = templateSpecification.types[0].supportedFlavors.map((flavor) => flavor.value);
...
async function handleTypeParameter(type, app, templateSpecifications) {
    if (!type)
        return;
    ...
    templateSpecification.types.forEach((spec) => {
        if (limitReached(app, spec)) {
            throw new AbortError('Invalid extension type', `You can only generate ${spec.registrationLimit} extension(s) of type ${spec.externalIdentifier} per app`);
        }
    });
